...
export default function RFIFormCreation() {
    ...
    const { handleSubmit, handleProjectChange, handleCreatedByEmployeeChange, handleSentByEmployeeChange } = useRFIFormHandler({setCurrentRFIData, navigate, setErrorString, method: "POST"})
...
function RFIFormBase({errorString, currentRFIData, handleSubmit, handleProjectChange, handleCreatedByEmployeeChange, handleSentByEmployeeChange, projects, employees, method }: RFIFormBaseProps){
    ...
    <GenericForm form_id="rfi_form" onSubmit={handleSubmit}>
        ...
        <SelectionComponent label="Assigned To" name="user" Value={currentRFIData.sent_by.name} options={employees} onChange={handleSentByEmployeeChange}/>
        <SelectionComponent label="Sent By" name="sent_by" Value={currentRFIData.created_by.name} options={employees} onChange={handleCreatedByEmployeeChange}/>
        ...
        <GenericInput label="Notes Closed" name="notes_closed" type="text" value={currentRFIData.notes_closed} />
        
        <BottomFormButton button_text={method === "POST" ? "Create RFI" : "Update RFI"}/>
